  ...
  function SalesChart({bookings, numDays}){
    ...
    const data = allDates.map((date)=>{
      return({
        ...
        total : bookings.filter((bookings)=>isSameDay(date, new Date(bookings.created_at))).reduce((acc, cur)=>acc+cur.total, 0),
        extras : bookings.filter((bookings)=>isSameDay(date, new Date(bookings.created_at))).reduce((acc, cur)=>acc+cur.extra_price, 0),
      });
    })
